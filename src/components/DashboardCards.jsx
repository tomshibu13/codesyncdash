...
export default function DashboardCards() {
  ...
  return (
    <div className={`space-y-6 transition-all duration-300 ${!isDarkMode ? 'p-6 rounded-3xl bg-slate-100/95 border border-slate-300 shadow-2xl shadow-slate-300/60' : ''
      ...
      <div className={`p-6 rounded-3xl border transition-all duration-300 ${isDarkMode ? 'bg-slate-900/80 border-slate-800/80' : 'bg-white border-slate-200 shadow-lg'
        }`}>
            ...
            return (
              <React.Fragment key={step.id}>
                ...
                >
                  {/* Subtle Inner Glow */}
                  {isComplete && (
                    <div className="absolute top-0 right-0 w-28 h-28 bg-emerald-400/30 rounded-full blur-2xl pointer-events-none -mr-8 -mt-8" />
                  )}
                  ...
                  <div className="flex items-center justify-between gap-3 w-full relative z-10">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <h3 className={`text-sm sm:text-base font-black tracking-tight uppercase font-mono truncate ${isComplete ? (isDarkMode ? 'text-emerald-100' : 'text-emerald-950') : (isDarkMode ? 'text-slate-100' : 'text-slate-800')
                        }`}>
                        {step.name}
                      </h3>
                    </div>
                  ...
                  <div className="relative z-10 pl-11">
                    <span className={`text-[10px] sm:text-[11px] font-bold uppercase font-mono tracking-wider block whitespace-nowrap ${isComplete ? (isDarkMode ? 'text-emerald-300 font-extrabold' : 'text-emerald-800 font-extrabold') : 'text-slate-500'
                      }`}>
