...
export default function DashboardCards() {
  ...
  return (
    <div className={`space-y-6 transition-all duration-300 ${!isDarkMode ? 'p-6 rounded-3xl bg-slate-100/95 border border-slate-300 shadow-2xl shadow-slate-300/60' : ''
      }`}>
        ...
        {mainCardsData.map((card, index) => {
          ...
          return (
            <motion.div
              ...
              className={`relative p-6 sm:p-7 rounded-3xl bg-gradient-to-b ${isDarkMode ? card.colorDark : card.colorLight
                } ${isDarkMode
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3 mb-4 relative z-10">
                <span className={`text-sm font-bold uppercase tracking-wider leading-snug ${isDarkMode ? 'text-slate-200' : 'text-slate-700'
                  }`}>
                  {card.title}
                </span>
                <div className={`p-3 rounded-2xl border shrink-0 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300 shadow-md ${isDarkMode ? card.iconBgDark : card.iconBgLight
                  }`}>
              ...
              <div className="my-2 relative z-10">
                <div className={`text-4xl sm:text-5xl font-black tracking-tight font-mono ${isDarkMode ? 'text-slate-100' : 'text-slate-900'
                  }`}>
                  {card.value}
                </div>
              </div>
              ...
              {/* Footer / Trend */}
              <div className={`flex items-center justify-between text-xs pt-3.5 border-t mt-3 gap-2 relative z-10 ${isDarkMode ? 'border-slate-800/80 text-slate-400' : 'border-slate-200 text-slate-600'
                }`}>
      ...
      <div className={`flex items-center justify-between pt-5 pb-2 border-b ${isDarkMode ? 'border-slate-800/80' : 'border-slate-300'
        }`}>
        ...
        {otherCardsData.map((card, index) => {
          ...
          return (
            <motion.div
              ...
              className={`relative p-4 rounded-2xl bg-gradient-to-b ${isDarkMode ? card.colorDark : card.colorLight
                } ${isDarkMode
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2 mb-2">
                <span className={`text-xs font-semibold uppercase tracking-wider leading-snug ${isDarkMode ? 'text-slate-300' : 'text-slate-700'
                  }`}>
                  {card.title}
                </span>
                <div className={`p-1.5 rounded-xl border shrink-0 group-hover:scale-110 transition-transform ${isDarkMode ? card.iconBgDark : card.iconBgLight
                  }`}>
              ...
              {/* Main Value */}
              <div className="my-1">
                <div className={`text-2xl font-black tracking-tight font-mono ${isDarkMode ? 'text-slate-100' : 'text-slate-900'
                  }`}>
                  {card.value}
                </div>
              </div>
              ...
              {/* Footer / Trend */}
              <div className={`flex items-center justify-between text-xs pt-2 border-t mt-2 gap-2 ${isDarkMode ? 'border-slate-800/60 text-slate-400' : 'border-slate-200 text-slate-600'
                }`}>
                <span className="leading-tight">{card.desc}</span>
                <span className={`inline-flex items-center gap-0.5 font-medium text-[10px] px-1.5 py-0.5 rounded-md shrink-0 ${card.trendUp
